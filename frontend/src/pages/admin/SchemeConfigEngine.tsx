...
export const SchemeConfigEnginePage: React.FC = () => {
  ...
  const [objectives, setObjectives] = useState(
    '• Provide financial assistance to ST researchers to complete advanced research without economic hardship.\n• Increase the representation of ST faculty and research scientists in national universities.\n• Support research on tribal culture, languages, indigenous knowledge, and advanced STEM fields.'
  );
  const [applicantGuidelines, setApplicantGuidelines] = useState(
    '1. Upload clear, legible PDF copies of all required certificates.\n2. Ensure applicant name on certificates matches the registered profile name exactly.\n3. The annual family income certificate must be from the competent revenue authority.\n4. Bonafide certificate must be signed by the Dean or Registrar of the University.'
  );
      ...
      <div className="bg-white p-6 rounded-2xl border border-[#c9b79c] shadow-xs flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-1">
            Configure scheme metadata, write detailed applicant descriptions, and choose mandatory verification documents.
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <select
            ...
            className="px-3 py-2 bg-[#f1e0c5] border border-[#c9b79c] rounded-xl text-xs font-bold text-[#2c352a] focus:ring-2 focus:ring-[#71816d] outline-hidden cursor-pointer"
          >
            <option value="new">+ Create New Scheme</option>
            {existingSchemes.map((s) => (
              <option key={s.id || s.code} value={s.id || s.code}>
                Edit: {s.code} - {s.name.slice(0, 28)}...
              </option>
            ))}
          </select>
          ...
          <button
            ...
            disabled={saving}
            className="inline-flex items-center px-5 py-2.5 rounded-xl bg-orange-500 hover:bg-orange-600 text-white font-bold text-xs shadow-md transition-all cursor-pointer disabled:opacity-50"
          >
            <Save className="w-4 h-4 mr-2" />
            <span>{saving ? 'Publishing...' : selectedSchemeId === 'new' ? 'Publish New Scheme Live' : 'Update Scheme Live'}</span>
          </button>
        </div>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        {/* Left Panel: Configuration Builder with Tabs */}
        <div className="lg:col-span-7 bg-white p-6 rounded-2xl border border-[#c9b79c] shadow-xs space-y-6">
          {/* Tab Navigation */}
          <div className="flex flex-wrap items-center gap-2 border-b border-[#dfcdb1] pb-3">
            <button
              type="button"
            ...
            >
              ...
              <span>2. Description & Guidelines</span>
            </button>
            ...
            >
              ...
              <span
                className={`ml-1.5 text-[10px] px-1.5 py-0.2 rounded-full font-extrabold ${
                  activeTab === 'documents' ? 'bg-white/20 text-white' : 'bg-[#dfcdb1] text-[#2c352a]'
                }`}
              >
                {requiredDocs.length}
              </span>
            ...
            >
              ...
              <span>4. Eligibility Rules</span>
              <span
                className={`ml-1.5 text-[10px] px-1.5 py-0.2 rounded-full font-extrabold ${
                  activeTab === 'rules' ? 'bg-white/20 text-white' : 'bg-[#dfcdb1] text-[#2c352a]'
                }`}
              >
                {rules.length}
              </span>
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <h3 className="font-bold text-slate-900 text-sm flex items-center">
                  <Sliders className="w-4 h-4 mr-2 text-[#71816d]" />
                  Scheme Metadata & Financials
                </h3>
                <span className="text-[11px] text-slate-500">Step 1 of 4</span>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
                <div>
                  ...
                  <input
                    type="text"
                    value={schemeName}
                    placeholder="e.g. National Fellowship for Higher Education of ST Students"
                ...
                <div>
                  ...
                  <input
                    ...
                    className="mt-1 block w-full px-3 py-2 border border-slate-300 rounded-lg text-xs uppercase font-mono font-bold focus:ring-2 focus:ring-[#71816d] outline-hidden"
                  />
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    className="mt-1 block w-full px-3 py-2 border border-slate-300 rounded-lg text-xs font-bold focus:ring-2 focus:ring-[#71816d] outline-hidden"
                  />
                </div>
              ...
              <div className="pt-4 border-t border-[#dfcdb1] flex justify-end">
                <button
                  type="button"
                  onClick={() => setActiveTab('description')}
                  className="px-4 py-2 bg-[#71816d] hover:bg-[#8c9c88] text-white text-xs font-bold rounded-xl cursor-pointer"
                >
          ...
          {activeTab === 'description' && (
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <h3 className="font-bold text-slate-900 text-sm flex items-center">
                  <AlignLeft className="w-4 h-4 mr-2 text-[#71816d]" />
                  Scheme Description & Applicant Guidelines
                </h3>
              ...
              <div className="space-y-4 text-xs">
                ...
                <div>
                  <label className="block font-semibold text-slate-700">
                    Short Tagline / Summary (Shown on scheme browsing cards)
                  </label>
                  <input
                    type="text"
                    value={shortSummary}
                ...
                <div>
                  <label className="block font-semibold text-slate-700">
                    Full Scheme Description (Detailed overview for applicants) *
                  </label>
                  <textarea
                    rows={4}
                    value={description}
                    placeholder="Describe the scheme scope, target beneficiaries, eligibility highlights, and institutions covered..."
                    onChange={(e) => setDescription(e.target.value)}
                    className="mt-1 block w-full px-3 py-2.5 border border-slate-300 rounded-lg text-xs leading-relaxed focus:ring-2 focus:ring-[#71816d] outline-hidden"
                  />
                  <span className="text-[10px] text-slate-500 mt-1 block">
                    Character count: {description.length} characters
                  </span>
                </div>
                ...
                <div>
                  <label className="block font-semibold text-slate-700">
                    Key Objectives & Benefits (Bullet points or paragraph)
                  </label>
                  <textarea
                    rows={3}
                    value={objectives}
                    ...
                    className="mt-1 block w-full px-3 py-2 border border-slate-300 rounded-lg text-xs font-mono focus:ring-2 focus:ring-[#71816d] outline-hidden"
                  />
                </div>
                ...
                <div>
                  ...
                  <textarea
                    ...
                    placeholder="1. Documents must be in PDF format&#10;2. Must possess valid caste certificate issued by competent authority"
              ...
              <div className="pt-4 border-t border-[#dfcdb1] flex justify-between">
                <button
                  type="button"
                  onClick={() => setActiveTab('metadata')}
                  className="px-4 py-2 bg-[#f1e0c5] hover:bg-[#e8d6ba] text-slate-800 text-xs font-bold rounded-xl cursor-pointer"
                >
          ...
          {activeTab === 'documents' && (
            <div className="space-y-5">
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="font-bold text-slate-900 text-sm flex items-center">
                    <FolderPlus className="w-4 h-4 mr-2 text-[#71816d]" />
                    Choose Required Verification Documents
                  </h3>
                  <p className="text-[11px] text-slate-500 mt-0.5">
                    Select standard MoTA verification certificates or add custom document requirements for applicants.
                  </p>
                </div>
                <span className="text-[11px] text-slate-500">Step 3 of 4</span>
              </div>

              {/* Standard Presets Selector */}
              <div className="p-4 rounded-xl bg-[#fbf8f3] border border-[#dfcdb1] space-y-2.5">
                <span className="text-xs font-bold text-slate-900 block">
                  Quick Select MoTA Standard Verification Presets
                </span>
                <p className="text-[11px] text-slate-600">
                  Click on any certificate to toggle it on or off for this scheme:
                </p>
              ...
              <div className="p-4 rounded-xl bg-[#f1e0c5] border border-[#c9b79c] space-y-3">
                ...
                <div className="flex gap-2">
                  <input
                    type="text"
                    value={customDocName}
                    placeholder="e.g. Research Synopsis / Guide Recommendation / Gate Scorecard"
                    ...
                    className="flex-1 px-3 py-2 bg-white border border-slate-300 rounded-lg text-xs focus:ring-2 focus:ring-[#71816d] outline-hidden"
                  />
                  <button
                    type="button"
                    onClick={handleAddCustomDoc}
                    className="inline-flex items-center px-4 py-2 rounded-lg bg-[#71816d] hover:bg-[#8c9c88] text-white font-bold text-xs shadow-xs cursor-pointer"
                  >
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-bold text-slate-900">
                    Configured Documents for Applicants ({requiredDocs.length})
                  </span>
                  <span className="text-[10px] text-slate-500">Applicants must upload .PDF format for each</span>
                </div>

                {requiredDocs.length === 0 ? (
                  <div className="p-6 text-center rounded-xl border border-dashed border-rose-300 bg-rose-50 text-rose-700 text-xs font-semibold">
                    No documents selected. Please select or add at least 1 required document above.
                  </div>
                ) : (
                  <div className="space-y-2">
                    {requiredDocs.map((doc, idx) => (
                      <div
                        key={doc}
                        className="flex items-center justify-between p-3 rounded-xl bg-white border border-[#dfcdb1] shadow-xs"
                      >
                        <div className="flex items-center space-x-2.5">
                          ...
                          <div>
                            ...
                            <span className="ml-2 text-[10px] font-semibold text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded">
                              PDF Only • AI OCR Verified
                            </span>
                          </div>
                        </div>
                        ...
                        <button
                          ...
                          title="Remove document from required list"
                        >
              ...
              <div className="pt-4 border-t border-[#dfcdb1] flex justify-between">
                ...
                <button
                  ...
                  className="px-4 py-2 bg-[#71816d] hover:bg-[#8c9c88] text-white text-xs font-bold rounded-xl cursor-pointer"
                >
                  Continue to Eligibility Rules ({rules.length}) →
                </button>
              </div>
            </div>
          )}
          ...
          {activeTab === 'rules' && (
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="font-bold text-slate-900 text-sm flex items-center">
                    <ShieldCheck className="w-4 h-4 mr-2 text-[#71816d]" />
                    Dynamic Eligibility Rules Engine
                  </h3>
                  <p className="text-[11px] text-slate-500 mt-0.5">
                    Define mathematical rules that will automatically filter and score applicant submissions.
                  </p>
                </div>
                <button
                  type="button"
                  onClick={handleAddRule}
                  className="inline-flex items-center px-2.5 py-1 rounded-lg bg-[#e8d6ba] hover:bg-[#dfcdb1] text-slate-800 text-xs font-bold cursor-pointer"
                >
                  <Plus className="w-3.5 h-3.5 mr-1" />
                  <span>Add Rule</span>
                </button>
              ...
              <div className="space-y-2">
                {rules.map((rule, idx) => (
                  <div
                    key={rule.id || idx}
                    className="flex items-center space-x-2 p-2.5 bg-[#f1e0c5] border border-[#c9b79c] rounded-xl text-xs"
                  >
                    <input
                      type="text"
                      value={rule.field}
                      ...
                      className="w-1/3 px-2 py-1.5 bg-white border border-slate-300 rounded-md outline-hidden"
                    />
                    <select
                      value={rule.operator}
                      onChange={(e) => {
                      ...
                      className="w-1/4 px-2 py-1.5 bg-white border border-slate-300 rounded-md font-mono outline-hidden"
                    >
              ...
              <div className="pt-4 border-t border-[#dfcdb1] flex justify-between items-center">
                <button
                  type="button"
                  onClick={() => setActiveTab('documents')}
                  className="px-4 py-2 bg-[#f1e0c5] hover:bg-[#e8d6ba] text-slate-800 text-xs font-bold rounded-xl cursor-pointer"
                ...
                >
                  <Save className="w-4 h-4 mr-2" />
                  <span>{saving ? 'Publishing...' : 'Save & Publish Scheme Live'}</span>
                </button>
              </div>
            </div>
          )}
        ...
        <div className="lg:col-span-5 bg-white p-6 rounded-2xl border border-[#c9b79c] shadow-xs space-y-4">
          ...
          <div className="bg-[#fbf8f3] border border-[#dfcdb1] rounded-xl p-4 sm:p-5 space-y-3.5 text-xs">
            ...
            <div>
              ...
              {shortSummary && (
                <p className="text-[11px] font-medium text-[#71816d] mt-0.5 italic">{shortSummary}</p>
              )}
            ...
            {/* Required Documents Preview (Updated live from chooser) */}
            <div className="pt-2 border-t border-[#dfcdb1] space-y-2">
              <div className="flex items-center justify-between">
                <h5 className="font-bold text-[#2c352a] text-xs flex items-center gap-1.5">
                ...
                <span className="text-[10px] text-slate-500 font-medium">Uploaded as .pdf only</span>
              </div>

              {requiredDocs.length === 0 ? (
                <p className="text-[11px] text-slate-400 italic">No documents selected yet.</p>
              ) : (
                <div className="space-y-1.5">
                  {requiredDocs.map((doc, idx) => (
                    <div
                      key={idx}
                      className="p-2 rounded-lg bg-white border border-[#dfcdb1] flex items-center justify-between text-[11px]"
                    >
                      <span className="font-medium text-slate-800 flex items-center">
                        <span className="w-1.5 h-1.5 rounded-full bg-[#71816d] mr-2 shrink-0"></span>
                        {doc}
                      </span>
                      <span className="text-[9px] font-bold text-amber-800 bg-[#f1e0c5] px-1.5 py-0.5 rounded border border-[#dfcdb1]">
                        Mandatory PDF
                      </span>
                    </div>
                  ))}
            ...
            {/* Objectives & Guidelines Preview if present */}
            {(objectives || applicantGuidelines) && (
              <div className="pt-2 border-t border-[#dfcdb1] space-y-2 text-[11px] text-slate-600">
                {objectives && (
                  <div>
                    <span className="font-bold text-[#2c352a] block text-xs">Objectives:</span>
                    <p className="whitespace-pre-line text-[10px] mt-0.5">{objectives}</p>
                  </div>
                )}
                {applicantGuidelines && (
                  <div>
                    <span className="font-bold text-[#2c352a] block text-xs">Applicant Guidelines:</span>
                    <p className="whitespace-pre-line text-[10px] mt-0.5">{applicantGuidelines}</p>
                  </div>
                )}
              </div>
            )}
